...
def _table_to_markdown(headers: list[str], rows: list[list[str]]) -> str:
    ...
    if not headers and rows:
        logger.debug("Markdown table rows length: %s", len(rows))
        logger.debug("Accessing markdown table first row index: 0")
        ...
        # ── TABLE → extract and attach to current section ───────────────────
            ...
            try:
                ...
                if table_data and hasattr(table_data, "grid"):
                    ...
                    if grid:
                        logger.debug("Docling table grid length: %s", len(grid))
                        logger.debug("Accessing docling table grid index: 0")
